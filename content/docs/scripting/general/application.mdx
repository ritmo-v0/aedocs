...
[Viewer object](../other/viewer.mdx) object; read-only.
...
[Fonts object](../text/fontsobject.mdx); read-only.
...
<Callout type="idea">`$.locale` returns the operating system language, not the language of the After Effects application.</Callout>
...
The currently loaded AE app preferences. See [Preferences object](../other/preferences.mdx).
...
The project that is currently loaded. See [Project object](project.mdx).

#### Type

[Project object](./project.mdx); read-only.
...
The currently loaded settings. See [Settings object](../other/settings.mdx).
...
Use `app.project.close(CloseOptions.DO_NOT_SAVE_CHANGES)` to close the current project before opening a new one. See [Project.close()](project.mdx#projectclose)
...
A new [Project object](./project.mdx), or `null` if no new project is created.
...
A new [Project object](./project.mdx) for the specified project, or `null` if the user cancels the Open dialog box.
...
A new [Project object](./project.mdx) for the specified project.
...
| `SwatchValue.a`     |                                                                                                           |
| `SwatchValue.b`     | - `L` is in the range `[0.0..1.0]` - `a` and `b`are in the range `[-128.0..+128.0]` `[0, 0, 0]` is Black. |
...
<Callout type="idea">This functionality was updated in After Effects 24.3 to allow the `ALL_CACHES` enumerated value to clear both the RAM and disk cache, with the ALL_MEMORY_CACHES enumerated value added to purge only the RAM. In versions prior to 24.3, `ALL_CACHES` will only clear the RAM cache.</Callout>
