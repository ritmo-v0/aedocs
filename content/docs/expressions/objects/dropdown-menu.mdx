...
On this page, `effect("Dropdown Menu Control")("Menu")` is used as a sample on how to call these items; however, note that any method that returns a dropdown menu [Property](../objects/property) will work.
...
This is effectively the same as how `valueAtTime()` relates to `value`, in that `text` will always return the dropdown string at the current time, and `textAtTime()` can return the string at a specific time.
