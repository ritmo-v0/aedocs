...
| `postEffect` | Boolean                    | Optional. If `true`, to sample the values _after_ layer masks and effects **directly applied to the layer** have been rendered. If `false`, sample values _before_ layer masks and effects. Defaults to `true`. |
