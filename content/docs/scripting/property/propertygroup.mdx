...
For example, you can only add a mask to a mask group. The only legal input arguments are "mask" or "ADBE Mask Atom".
...
[Property object](property) representing the variable font axis.
...
| `name`    | String  | The name of the child property. This can be:<br /><br />- Any match name<br />- Any name in expression "parenthesis style" syntax, meaning the display name or the compact English name<br />- Any name in expression "intercap style" syntax.<br /><br />For supported property names, see the table below. |
...
| From a 3D layer                         | - `"Accepts Shadows`" or `"acceptsShadows`"<br />- `"Accepts Lights`" or `"acceptsLights`"<br />- `"Ambient`" or `"ambient`"<br />- `"Diffuse`" or `"diffuse`"<br />- `"Specular`" or `"specular`" (these are for the Specular Intensity property) - `"Shininess`" or `"shininess`" (these are for the Specular Shininess property) - `"Casts Shadows`" or `"castsShadows`"<br />- `"Light Transmission`" or `"lightTransmission`"<br />- `"Metal`" or `"metal`" |
