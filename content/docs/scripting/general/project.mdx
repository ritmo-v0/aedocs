...
[Item object](../item/item.mdx) or `null`; read-only.
...
<Callout>This functionality was added in After Effects 16.0 (CC 2019)</Callout>
...
<Callout>This functionality was added in After Effects 17.5 (CC2020).</Callout>

<Callout type="warn">This method/property is officially undocumented and was found via research. The information here may be inaccurate, and this whole method/property may disappear or stop working some point. Please contribute if you have more information on it!</Callout>
...
<Callout type="warn">Setting this attribute to `FramesCountType.FC_TIMECODE_CONVERSION` resets the `displayStartFrame` attribute to 0.</Callout>
...
<Callout>This functionality was added in After Effects 13.8 (CC 2015.3)</Callout>

#### Description

Get or set the current projects GPU Acceleration option.
see [app.availableGPUAccelTypes](application.mdx#appavailablegpuacceltypes)
...
[ItemCollection object](../item/itemcollection.mdx); read-only.
...
The [Render Queue](../renderqueue/renderqueue.mdx) of the project.

#### Type

[RenderQueue object](../renderqueue/renderqueue.mdx); read-only.
...
[FolderItem object](../item/folderitem.mdx); read-only.
...
Array of [Item objects](../item/item.mdx); read-only.
...
<Callout>This functionality was added in After Effects 14.0 (CC 2017)</Callout>
...
if (comp instanceof CompItem) {
	...
	if (app.project.toolType !== ToolType.Tool_CameraMaya && app.project.toolType !== ToolType.Tool_CameraOrbit && app.project.toolType !== ToolType.Tool_CameraTrackXY && app.project.toolType !== ToolType.Tool_CameraTrackZ) {
...
<Callout>This functionality was added in After Effects 24.5</Callout>
...
Each object is composed of `font` which is a [Font object](../text/fontobject.mdx), and `usedAt` which is an Array of Objects, each composed of `layerID`, a [Layer.id](../layer/layer.mdx#layerid), and `layerTimeD` for when. See [Project.layerByID()](#projectlayerbyid) to retrieve the layers.
...
<Callout type="idea">When the project's color working space is set, the working gamma value is ignored by After Effects.</Callout>
...
| Parameter       | Type                                        | Description                                                              |
| --------------- | ------------------------------------------- | ------------------------------------------------------------------------ |
| `importOptions` | [ImportOptions](../other/importoptions.mdx) | Options specifying the file to import and the options for the operation. |
...
Array of [Item objects](../item/item.mdx) created during import; or `null` if the user cancels the dialog box.
...
<Callout>This functionality was added in After Effects 13.0 (CC 2014)</Callout>

#### Description

Retrieves an item by its [Item ID](../item/item.mdx#itemid)
...
[Layer object](../layer/layer.mdx) with the given ID if it exists on the project; otherwise null. Non-valid IDs will throw an exception stating that the input parameter is not an unsigned integer.
...
| Parameter        | Type                                      | Description           |
| ---------------- | ----------------------------------------- | --------------------- |
| `array_of_items` | Array of [Item objects](../item/item.mdx) | The items to be kept. |
...
This function will replace all the usages of [Font object](../text/fontobject.mdx) `fromFont` with [Font object](../text/fontobject.mdx) `toFont`.

This operation exposes the same mechanism and policy used for automatic font replacement of missing or substituted fonts and is therefore a complete and precise replacement, even on [TextDocuments](../text/textdocument.mdx) which have mixed styling, preserving the character range the `fromFont` was applied to.
...
| Parameter       | Type                                  | Description                   |
| --------------- | ------------------------------------- | ----------------------------- |
| `fromFont`      | [Font object](../text/fontobject.mdx) | Font to be replaced.          |
| `toFont`        | [Font object](../text/fontobject.mdx) | Font to replace it with.      |
