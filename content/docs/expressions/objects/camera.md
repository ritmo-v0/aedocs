...
1. The camera is the active camera for the composition at the current time: the _video switch_ for the camera layer is on,
2. the current time is in the range from the _in point_ of the camera layer to the _out point_ of the camera layer, **and**
3. The camera is the first (topmost) such camera layer listed in the _timeline panel_
