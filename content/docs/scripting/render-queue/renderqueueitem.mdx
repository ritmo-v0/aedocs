...
[CompItem object](../item/compitem.mdx); read-only.
...
You cannot make changes to render queue items or to the application while rendering is in progress or paused; you can, however, use this callback to pause or stop the rendering process. See [RenderQueue.pauseRendering()](renderqueue.mdx#renderqueuepauserendering) and [RenderQueue.stopRendering()](renderqueue.mdx#renderqueuestoprendering). See also [app.onError](../general/application.mdx#apponerror).
...
[OMCollection object](omcollection.mdx); read-only.
...
<Callout type="idea">Duplicating an item whose status is "Done" sets the new item's status to "Queued".</Callout>
