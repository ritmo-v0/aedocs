...
| `includeExtents` | Boolean | Optional. Only applies to shape layers and paragraph text layers. Defaults to `false`. - For shape layers: Increases the size of the layer bounds as necessary. - For paragraph text layers: Returns the bounds of the paragraph box |
