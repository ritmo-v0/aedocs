...
<Callout>Layer is a subclass of [PropertyGroup](../property/propertygroup.mdx), which is a subclass of [PropertyBase](../property/propertybase.mdx). All methods and attributes of PropertyGroup, in addition to those listed below, are available when working with Layer, with the exception that `propertyIndex` attribute is set to `undefined`.</Callout>

<Callout>Layer is the base class for [CameraLayer object](cameralayer.mdx), [LightLayer object](lightlayer.mdx), and [AVLayer object](avlayer.mdx), so Layer attributes and methods are available when working with all layer types. Layers contain AE properties, in addition to their JavaScript attributes and methods. For examples of how to access properties in layers, see [PropertyBase object](../property/propertybase.mdx).</Callout>

#### Example

If the first item in the project is a [CompItem](../item/compitem.mdx), this example disables the first layer in that composition and renames it. This might, for example, turn an icon off in the composition.
...
<Callout type="idea">This always returns `true` for layers that do not have a [AVLayer.source](avlayer.mdx#avlayersource)</Callout>
...
<Callout type="idea">Custom label colors cannot be set programmatically.</Callout>
...
A [PropertyGroup object](../property/propertygroup.mdx) that contains all a layer's markers. Layer marker scripting has the same functionality as [Comp markers](../item/compitem.mdx#compitemmarkerproperty).

See [MarkerValue object](../other/markervalue.mdx).
...
An array containing all of the currently selected [Property](../property/property.mdx) and [PropertyGroup](../property/propertygroup.mdx) objects in the layer.

#### Type

Array of [PropertyBase](../property/propertybase.mdx) objects; read-only.
...
<Callout type="warn">The animation preset is applied to the the selected layer(s) of the comp, not to the layer whose applyPreset function is called. Hence, the layer whose applyPreset function is called effectively just determines the comp whose layers are processed.</Callout>
...
Creates a new Layer object with the same values as this one, and prepends the new object to the [LayerCollection object](layercollection.mdx) in the target CompItem. Retrieve the copy using into `Comp.layer(1)`.
...
<Callout>As of After Effects 13.6, this method no longer causes After Effects to crash when the layer has a parent.</Callout>

<Callout type="warn">As of After Effects 13.7 (13.6, has not been tested), if the copied layer has an effect on it and the user undoes the action, After Effects will Crash.</Callout>

#### Parameters

| Parameter  | Type                                    | Description             |
| ---------- | --------------------------------------- | ----------------------- |
| `intoComp` | [CompItem object](../item/compitem.mdx) | The target composition. |
...
<Callout>This functionality was added in After Effects 22.3 (2022)</Callout>
...
| Parameter | Type                      | Description                               |
| --------- | ------------------------- | ----------------------------------------- |
| `layer`   | [Layer object](layer.mdx) | The target layer in the same composition. |
...
| Parameter   | Type                      | Description                                                                              |
| ----------- | ------------------------- | ---------------------------------------------------------------------------------------- |
| `newParent` | [Layer object](layer.mdx) | Optional. A layer in the same composition. If not specified, it sets the parent to None. |
