...
The FootageSource object holds information describing the source of some footage. It is used as the `mainSource` of a [FootageItem object](../item/footageitem.mdx), or the `proxySource` of a [CompItem object](../item/compitem.mdx) or FootageItem.

<Callout>FootageSource is the base class for [SolidSource object](solidsource.mdx), so FootageSource attributes and methods are available when working with SolidSource objects.</Callout>
