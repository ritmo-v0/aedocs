...
<Callout>AVItem is a subclass of Item. All methods and attributes of Item, in addition to those listed below, are available when working with AVItem. See [Item object](item.mdx)</Callout>

<Callout>AVItem is the base class for both CompItem and FootageItem, so AVItem attributes and methods are also available when working with CompItem and FootageItem objects. See [CompItem object](compitem.mdx) and [FootageItem object](footageitem.mdx).</Callout>

<Callout type="warn">CompItems and FootageItems, while logical descendants of AVItem, are not *really* subclasses of AVItem as AVItem doesn't exist in Extendscript, ie. attempting to check if `item instanceof AVItem` will fail because AVItem is undefined. This is also true for `Item` itself.</Callout>

See [Javascript Classes](../introduction/javascript.mdx#javascript-classes) and [After Effects Class Hierarchy](../introduction/classhierarchy.mdx) for more info.
...
Floating-point value, in the range `[0.0..10800.0]`; read/write for a [CompItem](../item/compitem.mdx); otherwise, read-only.
...
Floating-point value, in the range `[1/99..1.0]`; read-only for a [FootageItem](../item/footageitem.mdx), otherwise read/write.
...
Floating-point value, in the range `[1.0..99.0]`; read-only for a [FootageItem](../item/footageitem.mdx), otherwise read/write.
...
Test whether the AVItem can be used as an alternate source when calling [Property.setAlternateSource()](../property/property.mdx#propertysetalternatesource).
...
- If the AVItem is a [FootageItem object](footageitem.mdx), then FootageItem.FootageSource should not be a [SolidSource object](../sources/solidsource.mdx).
- If the AVItem is a [FootageItem object](footageitem.mdx) and the FootageItem.FootageSource is a [FileSource object](../sources/filesource.mdx) then that FileSource should not point to a non-media file e.g. a JSX script file.
...
The current time of the item when it is being previewed directly from the Project panel. This value is a number of seconds. Use the global method [timeToCurrentFormat()](../general/globals.mdx#timetocurrentformat) to convert it to a string value that expresses the time in terms of frames. It is an error to set this value for a FootageItem whose `mainSource` is still (`item.mainSource.isStill` is `true`).
...
<Callout>There is no direct way to set a placeholder as a proxy in the user interface; this behavior occurs when a proxy has been set and then moved or deleted.</Callout>
...
Creates a [SolidSource object](../sources/solidsource.mdx) with specified values, sets this as the value of the `proxySource` attribute, and sets `useProxy` to `true`. It does not preserve the interpretation parameters, instead using the user preferences.

<Callout>There is no way, using the user interface, to set a solid as a proxy; this feature is available only through scripting.</Callout>
