...
<Callout type="idea">The feather points on a mask are listed in an array in the order that they were created.</Callout>
...
<Callout type="idea">Values are stored in the array in the order that feather points are created.</Callout>
...
<Callout type="idea">Values are stored in the array in the order that feather points are created. To move a feather point to a different mask path segment, first change the [featherSegLocs](#shapefeatherseglocs) attribute value, then this attribute.</Callout>
...
<Callout type="idea">Values are stored in the array in the order that feather points are created. Move a feather point to a different segment by changing both its segment number (this attribute) and, optionally, its [featherRelSegLocs](#shapefeatherrelseglocs) attribute value.</Callout>
...
<Callout type="idea">You cannot change the direction of a feather point after it has been created. Values are stored in the array in the order that feather points are created.</Callout>
