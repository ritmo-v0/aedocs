...
<Callout>This functionality was added in After Effects 24.0</Callout>
...
The properties we report via the proxy [Font object](fontobject.mdx) are the data that is available to us from the font files themselves, which of course will vary according to technology and type of font. It is not possible here to describe all the possible interesting variations and troubles that this causes us and in general it is advisable to be careful with assuming that the behavior and properties for one font type or technology are common to all other font types and technology - the answer as always is "it depends".

A [Font object](fontobject.mdx) is a soft reference to one of these internal font proxies and as a consequence is not sufficient to keep the internal font proxy alive. As a result if the internal font proxy is removed, the referencing [Font object](fontobject.mdx) will throw an invalid exception for any property reference.

On project open, and a few other situations, it may come to pass that the font which is being referenced in the persisted data cannot be found in the current font ecosystem. In these situations an internal font proxy will be created which will contain the desired properties, such as PostScript name, and will return `true` for [isSubstitute](fontobject.mdx#fontobjectissubstitute). There will be an underlying real font which will be selected to support this internal font proxy, but we do not reveal what it is and there is no way to influence this selection.
...
They are grouped into what is named a family group which are Arrays of [Font object](fontobject.mdx).

The Family Name of the group is simply the [familyName](fontobject.mdx#fontobjectfamilyname) of any of the [Font objects](fontobject.mdx) in the group.

The Family Name in one font group is not guaranteed to have unique name compared to different font groups - the grouping is determined by a number of factors including the returned value of [FontObject.technology](fontobject.mdx#fontobjecttechnology) and [FontObject.writingScripts](fontobject.mdx#fontobjectwritingscripts).
...
The family groups and [Font objects](fontobject.mdx) in the group are sorted according to the setting in the Character Panel dropdown "Show Font Names in English". If set to `true`, the [familyName](fontobject.mdx#fontobjectfamilyname) and [styleName](fontobject.mdx#fontobjectstylename) property is used, otherwise the [nativeFamilyName](fontobject.mdx#fontobjectnativefamilyname) and [nativeStyleName](fontobject.mdx#fontobjectnativestylename) property is used.

[Font object](fontobject.mdx) for which [isSubstitute](fontobject.mdx#fontobjectissubstitute) returns `true` are always sorted to the end as individual family groups.

#### Type

Array of Arrays of [Font objects](fontobject.mdx); read-only.
...
<Callout>This functionality was added in After Effects 24.6</Callout>

#### Description

Provides access to the Favorites list presented in the Character panel and Properties panel. To set the Favorites simply provide an (unsorted) array of strings based on the [familyName](fontobject.mdx#fontobjectfamilyname). To clear the list simply assign an empty Array.
...
It is perfectly legal and common for more than one [Font object](fontobject.mdx) to return the same value for [postScriptName](fontobject.mdx#fontobjectpostscriptname) but as this can sometimes lead to confusion about what [Font object](fontobject.mdx) will actually be used when using [TextDocument.font](textdocument.mdx#textdocumentfont) or the `.font` attribute of a [CharacterRange object](characterrange.mdx), this property exists to both reveal what duplicates exist and also their relative order.

This returns an Array in which each element is an Array of [Font objects](fontobject.mdx), where the 0th element [Font object](fontobject.mdx) is considered the primary [Font object](fontobject.mdx) for the given PostScript name.
...
<Callout>This functionality was added in After Effects 24.2</Callout>

#### Description

Returns an unsigned number representing the current revision of the font environment.

The revision is advanced when anything happens to the font environment which would change the contents, properties, or order of [Font objects](fontobject.mdx) returned from a call to [FontsObject.allFonts](#fontsobjectallfonts).
...
Returns an array of variable [Font objects](fontobject.mdx), each using a unique font dictionary and with default values for their design axes. This API is a convenient way to quickly filter for a unique instance of each installed variable font.

#### Type

Array of [Font objects](fontobject.mdx); read-only.
...
When a Project is opened and one or more fonts are not found in the local font environment, a _sync_ process is initiated with Adobe Fonts to see if one or more Fonts could be activated and installed.
...
<Callout type="idea">A substituted font is a font that was already missing when the project is opened. A missing font is a font that went missing (font being uninstalled, for example) *while* the project was open.</Callout>
...
Provides access to the Most Recently Used (MRU) list presented in the Character panel and Properties panel. To set the MRU simply provide an (unsorted) array of strings based on the [familyName](fontobject.mdx#fontobjectfamilyname). To clear the list simply assign an empty Array.
...
Controls the rules which are used to determine which fonts are considered matching for automatic replacement for a substituted [Font object](fontobject.mdx).

#### Type

A `SubstitutedFontReplacementMatchPolicy` enumerated value; read/write. One of:

- `SubstitutedFontReplacementMatchPolicy.POSTSCRIPT_NAME` is the default; any [Font object](fontobject.mdx) which has the same PostScript name is a valid candidate for replacement of a substituted [Font object](fontobject.mdx).
- `SubstitutedFontReplacementMatchPolicy.CTFI_EQUAL` requires that the following properties of substituted [Font object](fontobject.mdx) must match to be considered a valid candidate:
    - [postScriptName](fontobject.mdx#fontobjectpostscriptname)
    - [technology](fontobject.mdx#fontobjecttechnology)
    - [writingScripts](fontobject.mdx#fontobjectwritingscripts) (primary)
    - [designVector](fontobject.mdx#fontobjectdesignvector)
- `SubstitutedFontReplacementMatchPolicy.DISABLED` means that no [Font object](fontobject.mdx) is an acceptable replacement for a the substituted [Font object](fontobject.mdx).
...
<Callout>This functionality was added in After Effects 25.1</Callout>

#### Description

This function will return an array of generic objects describing the number of characters in the range and the `CTScript` enum assigned to them. See [FontObject.writingScripts](fontobject.mdx#fontobjectwritingscripts) for a full list of `CTScript` enumerated values.
...
var str =
	...
	scriptsV[0].chars + // 2
	" ctScript:" +
	getEnumAsString(scriptsV[0].ctScript) +
	"\n[1] chars:" +
	scriptsV[1].chars + // 1
	" ctScript:" +
	getEnumAsString(scriptsV[1].ctScript);
alert(str);
```
...
This function will return an instance of [Font object](fontobject.mdx) mapped as the default font based on `CTScript`.
...
This function will return an instance of [Font object](fontobject.mdx) based on the ID of a previously found font.
...
[Font object](fontobject.mdx), or undefined if no font by that ID.
...
This function will return an array of [Font object](fontobject.mdx) based on the Family Name and Style Name of a font. If no suitable font is found, it will return an empty Array.

<Callout type="idea">The returned array length can be more than 1 if you have multiple copies of a same font.</Callout>
...
This function will return an array of [Font objects](fontobject.mdx) based on the PostScript name of previously found Fonts.

It is perfectly valid to have multiple [Font objects](fontobject.mdx) which share the same PostScript name, the order of these is determined by the order in which they were enumerated in the font environment. The entry at `[0]` will be used when setting the [TextDocument.fontObject](textdocument.mdx#textdocumentfontobject) property.

In addition, there is a special property of this API with regards to Variable fonts. If no [Font object](fontobject.mdx) matching the requested PostScript exists, but we find that there exist a variable font which matches the requested PostScript name prefix, then this Variable font instance will be requested to create a matching [Font object](fontobject.mdx). This is the only way that we will return an instance which did not exist prior to invoking this method.
...
The addition and removal of font files in what is considered the _system font folders_ is recognized and handled automatically without user intervention to update the font environment. Non-system font folders are not automatically handled and so additions and removal of font files in these folders are not recognized until the After Effects is restarted.
...
This function will set an instance of [Font object](fontobject.mdx) mapped based on `CTScript` parameter.
...
| `font`     | [Font object](fontobject.mdx) | The font to be mapped. If `null`, then current mapping is reset. |
