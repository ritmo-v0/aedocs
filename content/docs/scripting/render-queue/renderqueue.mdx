...
A collection of all items in the render queue. See [RenderQueueItem object](renderqueueitem.mdx).

#### Type

[RQItemCollection object](rqitemcollection.mdx); read-only.
...
- To respond to errors during the rendering process, define a callback function in [app.onError](../general/application.mdx#apponerror).
- To respond to changes in the status of a particular item while the render is progressing, define a callback function in [RenderQueueItem.onstatus](renderqueueitem.mdx#renderqueueitemonstatus) in the associated RenderQueueItem object.
...
Stops the rendering process. This is the same as clicking Stop in the Render Queue panel during a render. You can call this method from an [RenderQueueItem.onstatus](renderqueueitem.mdx#renderqueueitemonstatus) or [app.onError](../general/application.mdx#apponerror) callback.
...
<Callout>This requires Adobe Media Encoder CC 2017 (11.0) or later.</Callout>

<Callout type="idea">When AME receives the queued items, it applies the most recently used encoding preset. If `render_immediately_in_AME` is set to `true`, you will not have an opportunity to change the encoding settings.</Callout>
