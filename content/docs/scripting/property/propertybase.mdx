...
See also [PropertyGroup.property()](propertygroup.mdx#propertygroupproperty).

<Callout>PropertyBase is the base class for both [Property](property.mdx) and [PropertyGroup](propertygroup.mdx), so PropertyBase attributes and methods are available when working with properties and property groups.</Callout>
...
This value is never `true` in an audio layer; there is a separate `audioActive` attribute in the AVLayer object [AVLayer.audioActive](../layer/avlayer.mdx#avlayeraudioactive).
...
For a layer, the name of the layer. By default, this is the same as the Source name, unless [Layer.isNameSet](../layer/layer.mdx#layerisnameset) returns `false`.
...
When `true`, this property is selected. Set to `true` to select the property, or to `false` to deselect it. Sampling this attribute repeatedly for a large number of properties can slow down system performance. To read the full set of selected properties of a composition or layer, use either [CompItem.selectedProperties](../item/compitem.mdx#compitemselectedproperties) or [Layer.selectedProperties](../layer/layer.mdx#layerselectedproperties).
...
<Callout type="warn">Using this method invalidates existing references to other children in the same indexed group. For example, if you have three effects on a layer, each effect assigned to a different variable, moving one of the effects invalidates the references for all of these variables. You will need to reassign them.</Callout>
...
PropertyGroup object, or [Layer object](../layer/layer.mdx) if the count reaches the containing layer.
