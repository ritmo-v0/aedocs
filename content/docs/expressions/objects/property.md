...
This category holds generic entries relating to generic _properties_.

In expressions, a Property is _typically_ anything that exists in the timeline (even if it can't be keyframed, or hold an expression).
...
To retrieve the _last_ keyframe on a property:
