...
The Fonts object provides information about the current font ecosystem on your device.
...
If a character is deemed to be included in one or more `CTScript` values, the value specified in the second argument `preferredCTScript` will break the tie.

```javascript
var scriptsV = app.fonts.getCTScriptForString("ABヂ", CTScript.CT_ROMAN_SCRIPT);
var str =
	"[0] chars:" + scriptsV[0].chars + // 2
...
There is no requirement that the specified font has glyphs for any or all of the characters
...
- `false` if the specified mapping is the same as the current one.
