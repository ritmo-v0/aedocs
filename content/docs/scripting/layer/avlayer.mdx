...
<Callout>AVLayer is a subclass of [Layer object](layer.mdx). All methods and attributes of Layer, in addition to those listed below, are available when working with AVLayer.</Callout>

<Callout>AVLayer is a base class for [TextLayer object](textlayer.mdx), so AVLayer attributes and methods are available when working with TextLayer objects.</Callout>
...
<Callout>This functionality was updated in After Effects 23.0. Track Matte is no longer dependent on layer order.</Callout>
...
<Callout>This functionality was added in After Effects 12.0 (CC)</Callout>
...
<Callout>This functionality was added in After Effects 23.0</Callout>
...
<Callout>This functionality was updated in After Effects 23.0</Callout>

<Callout type="warn">This is a Legacy API we don't recommend using for setting Track Matte Type in new scripts. Please consider using the latest track matte APIs [AVLayer.setTrackMatte()](#avlayersettrackmatte) and [AVLayer.removeTrackMatte()](#avlayerremovetrackmatte) for your tasks.</Callout>
...
| --------- | --------------------------------------- | -------------------------------------------------------------- |
| `comp`    | [CompItem object](../item/compitem.mdx) | The composition where you wish to add the property to the EGP. |
...
Media Replacement layers are recognized as AVLayers with an [AVLayer.source](#avlayersource) set to a [FootageItem object](../item/footageitem.mdx) (with specific source types) or a [CompItem object](../item/compitem.mdx).

The AVLayer needs to comply with the restrictions below in order to be treated as a Media Replacement layer:

- [Layer.hasVideo](../layer/layer.mdx#layerhasvideo) should return `true`.
- [AVLayer.adjustmentLayer](#avlayeradjustmentlayer) should return `false`.
- [Layer.nullLayer](../layer/layer.mdx#layernulllayer) should return `false`.
- If the [AVLayer.source](#avlayersource) is a [FootageItem object](../item/footageitem.mdx), then FootageItem.FootageSource should not be a [SolidSource object](../sources/solidsource.mdx).
- If the [AVLayer.source](#avlayersource) is a [FootageItem object](../item/footageitem.mdx) and the FootageItem.FootageSource is a [FileSource object](../sources/filesource.mdx) then that FileSource should not point to a non-media file e.g. a JSX script file.
...
<Callout type="warn">This value only reflects the first character in the text layer at the current time.</Callout>
...
<Callout type="warn">If this method is performed on a null layer, the layers `isNull` attribute is not changed from `true`. This causes the layer not to be visible in comp viewer and renders.</Callout>

#### Parameters

| Parameter        | Type                                | Description                                                                                                                                                                                                                                                                                          |
| ---------------- | ----------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `newSource`      | [AVItem object](../item/avitem.mdx) | The new source AVItem object.                                                                                                                                                                                                                                                                        |
| `fixExpressions` | Boolean                             | `true` to adjust expressions for the new source, otherwise `false`. **Warning:**This feature can be resource-intensive; if replacing a large amount of footage, do this only at the end of the operation. See also [Project.autoFixExpressions()](../general/project.mdx#projectautofixexpressions). |
...
| Parameter         | Type                                          | Description                                    |
| ----------------- | --------------------------------------------- | ---------------------------------------------- |
| `trackMatteLayer` | [AVLayer](../layer/avlayer.mdx)               | The layer to be used as the track matte layer. |
| `trackMatteType`  | [TrackMatteType](#avlayertrackmattetype) enum | The type of the track matte to be used.        |

<Callout type="warn">Passing in `TrackMatteType.NO_TRACK_MATTE` as type is invalid and will result in no-op.</Callout>
