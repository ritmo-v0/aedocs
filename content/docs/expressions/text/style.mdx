...
<Callout>
  When using per-character styling, line breaks and spaces are also considered characters and must be accounted for (or skipped, if desired) when calculating character indexes.
</Callout>
...
Returns `true` if the Text layer has Every-Line Composer set for the first paragraph of a Text layer and `false` if Single-Line Composer is set for the first paragraph of a Text layer
...
<Callout>
  ...
  Also, note that automatic kerning will take precedence over manual kerning.
</Callout>
